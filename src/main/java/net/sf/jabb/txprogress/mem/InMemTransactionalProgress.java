...
public class InMemTransactionalProgress implements TransactionalProgress {
	 ...
	 */
	void compact(LinkedList<BasicProgressTransaction> transactions){
		int finished = 0;
		Iterator<BasicProgressTransaction> iterator = transactions.iterator();
		if (iterator.hasNext()){
	...
	@Override
	public List<ReadOnlyProgressTransaction> getRecentTransactions(String progressId)
			throws InfrastructureErrorException {
		LinkedList<BasicProgressTransaction> transactions = progresses.get(progressId);
		synchronized(transactions){
			compact(transactions);
			ArrayList<ReadOnlyProgressTransaction> copy = new ArrayList<>(transactions.size());
			for (BasicProgressTransaction tx: transactions){

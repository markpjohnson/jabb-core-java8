 ...
 * For a progress, there can be more than one on-going transactions being handled by different processors at any time.
 * Therefore, there is no need for processors to acquire and hold leases on the whole progresses.
 * However, all transactions of any progress are continuous disregarding the reliability of the processors:
 * <ul>
 ...
 * 	<li>Multiple processors can work on different segments (specified by startPosition and endPosition) at the same time</li>
 ...
 * 
 * <ul>
 * 	<li>IDs of the transactions: They can be either generated by the service or specified by the client. 
 * 			They have to be unique within a reasonable time period for any progress but don't need to be unique across progresses.</li>
 * </ul>
 * 
 * Usage example:
 * <pre>
...
public interface TransactionalProgress {
	 ...
	 */
	default ProgressTransaction startTransaction(String progressId, String processorId, Instant timeout, int maxInProgressTransacions, int maxRetryingTransactions)
					throws InfrastructureErrorException{
		BasicProgressTransaction transaction = new BasicProgressTransaction();
		transaction.setProcessorId(processorId);
		transaction.setTimeout(timeout);
		try {
			return startTransaction(progressId, null, transaction, maxInProgressTransacions, maxRetryingTransactions);
	 ...
	 * @throws InfrastructureErrorException if error in the underlying infrastructure happened
	 */
	default ProgressTransaction startTransaction(String progressId, ReadOnlyProgressTransaction transaction, int maxInProgressTransacions, int maxRetryingTransactions) 
			throws InfrastructureErrorException{
		try {
			return startTransaction(progressId, null, transaction, maxInProgressTransacions, maxRetryingTransactions);
		} catch (DuplicatedTransactionIdException e) {
